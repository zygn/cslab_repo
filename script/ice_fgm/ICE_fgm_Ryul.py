...
class Pure_Pursuit:
    def __init__(self):
        ...
        self.drive_param = "/vesc/high_level/ackermann_cmd_mux/input/nav_1" 
       # self.drive_param = "/vesc/low_level/ackermann_cmd_mux/input/navigation" 

        rospy.Subscriber('/scan', LaserScan, self.subCallback_scan, queue_size = 10)
        rospy.Subscriber('/vesc/odom', Odometry, self.Odome, queue_size = 10)
        self.drive_pub = rospy.Publisher(self.drive_param, AckermannDriveStamped, queue_size = 10 )

    def get_waypoint(self):
        file_wps = np.genfromtxt('./wp-test.csv',delimiter=',',dtype='float')
